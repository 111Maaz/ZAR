...
export function EmptyState({
  ...
  return (
    <div className={`flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50/50 text-center ${compact ? 'px-4 py-6' : 'px-6 py-12'}`}>
      <div className={`flex items-center justify-center rounded-full bg-gray-100 ${compact ? 'mb-2 h-10 w-10' : 'mb-4 h-12 w-12'}`}>
        <Inbox className={`${compact ? 'h-5 w-5' : 'h-6 w-6'} text-gray-400`} />
      </div>
      <h3 className={`${compact ? 'text-xs' : 'text-sm'} font-semibold text-gray-900`}>{title}</h3>
      {description && <p className={`mt-1 max-w-sm ${compact ? 'text-xs' : 'text-sm'} text-gray-500`}>{description}</p>}
